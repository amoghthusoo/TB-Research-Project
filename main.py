...
def imaging_study():
	...
	flags.append(isWhiteOnly(coOrdDict["ULS_flag"]))
	flags.append(isWhiteOnly(coOrdDict["URS_flag"]))
	flags.append(isWhiteOnly(coOrdDict["MLS_flag"]))
	flags.append(isWhiteOnly(coOrdDict["MRS_flag"]))
	flags.append(isWhiteOnly(coOrdDict["LLS_flag"]))
	flags.append(isWhiteOnly(coOrdDict["LRS_flag"]))
...
def case_details():
	...
	while True:
		...
		for j in range(offsetValue,22):
			...
			if ("CR" in imToString(coOrdDict["modality1"])):
